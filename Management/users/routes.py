...
def get_users(
    ...
    try:
        if current_user.type == "admin":
            ...
            user_data: List[Dict] = databases.get_user(db, row=list(User.model_fields.keys()), column="id", value=current_user.user_id)
            if user_data.__len__() == 0:
                return {"detail": "no user available", "status_code": 201}
            return {"payload": user_data, "status_code": 200}
