...
app.add_middleware(
    CORSMiddleware,
    allow_origins=[
        "http://localhost",  # Allow requests from localhost (default HTTP)
        "http://127.0.0.1",  # Allow requests from IPv4 loopback
        "http://localhost:3000",  # Allow requests from a specific port (e.g., React app)
    ],
...
@app.post("/admin_auth", tags=["Authentication"])
def admin(
          form_data: OAuth2PasswordRequestForm = Depends(),
          db: Connection = Depends(get_db_connection)):
    username: str = form_data.username
    password: str = form_data.password
    user_data: List[Dict] = get_user(db, column="email", value=username)
    if len(user_data) == 0:
        return HTTPException(status_code=405, detail="User doesn't exists")
    if user_data[0].get("type") == "user":
        return HTTPException(status_code=406, detail="Admin only")
